...
from ui_Up_Display import  Ui_Up_Display


class UpDispaly( QFrame):
    def __init__(self,parent=None):
        super().__init__(parent)    #调用父类构造函数
        self.ui=Ui_Up_Display()         #创建ui对象
...
#2# ==========事件处理函数===========================================
    def mousePressEvent(self,event):
        if event.button()==Qt.LeftButton and not self.ui.cbb_dispalyMethod.frameGeometry().contains(event.pos()):
            self.bool_darg=True
            self.point_mouseStart=event.globalPos()
            self.point_windowTopLeft=self.frameGeometry().topLeft()

    def mouseMoveEvent(self,event):
        if self.bool_darg and not self.ui.cbb_dispalyMethod.frameGeometry().contains(event.pos()):
